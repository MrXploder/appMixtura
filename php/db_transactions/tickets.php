...
$dbResourcesForAdminGetRequest = array(
	...
	"clients"					  => "SELECT * FROM `clientes` ORDER BY `id_cliente` ASC",
...
if($_SERVER['REQUEST_METHOD'] === 'GET'){
	try{
		if(empty($requestedId)){
			...
			$payLoad = DB::query("SELECT `tickets`.*, `clients`.`name` AS client_name, `operators`.`name` AS operator_name FROM `tickets` INNER JOIN `clients` ON `tickets`.`client_id` = `clients`.`id` INNER JOIN `operators` ON `tickets`.`operator_id` = `operators`.`id` WHERE `tickets`.`id` = %d ORDER BY `tickets`.`id` DESC", $requestedId)[0];
		}
